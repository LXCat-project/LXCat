...
export async function updateSection(
  ...
  if (status === "published") {
    return await createDraftSection(version, section, message, key);
  } else {
...
async function createDraftSection(version: string, section: CrossSection<string, string, import("@lxcat/schema/dist/core/data_types").LUT>, message: string, key: string) {
	// Add to CrossSection with status=='draft'
	const newStatus: Status = "draft";
	// For draft version = prev version + 1
	const newVersion = `${parseInt(version) + 1}`;
	const keyOfDraft = await insert_cs_with_dict(
		section,
		{}, // TODO pass as argument or fetch from key
		{}, // TODO pass as argument or fetch from key
		'', // TODO pass as argument or fetch from key
		newStatus,
		newVersion,
