...
import { listOrganizations, listUsers, OrganizationFromDB, UserFromDB } from '../../auth/queries';
...
const AdminUsers: NextPage<Props> = ({ me, users: initalUsers, organizations }) => {
    ...
    const updateOrganization = async (user: UserFromDB, organizationKey?: string) => {
        if (organizationKey) {
        ...
        <Layout>
            ...
            <table style={{ width: '100%' }}>
                ...
                <tbody>
                    {
                        users.map(u => {
                            const memberOf = organizations.find(o => u.organization === o.name)
                            return (
                                <tr key={u.email}>
                                    <td>{u.name}</td>
                                    <td>{u.email}</td>
                                    ...
                                    <td>
                                        <select
                                            onChange={(event) => updateOrganization(u, event.target.value)}
                                            value={memberOf?._key}
                                        >
                                            <option></option>
                                            {organizations.map(o =>
                                                <option key={o._key} value={o._key}>{o.name}</option>
                                            )}
                                        </select>
                                    </td>
                                    <td>
